...
회원은 변경된 약관에 동의하지 않을 경우 회원 탈퇴를 요청할 수 있으며, 변경된 약관의 효력 발생일 이후 해지 요청을 하지 않을 경우 약관의 변경 사항에 동의한 것으로 간주됩니다.<br /><br />
제3조. 약관 외 준칙
...
"아이디" : 회원의 식별과 회원의 서비스 이용을 위하여 회원이 선정하고 회사가 승인하는 영문이나 숫자 혹은 그 조합을 말합니다(이하"ID"라 합니다). 2011년 11월 1일부터 2012년 10월 31일까지 통합 절차를 거친 회원은 ID 통합 절차를 통해 결정된 ID만 사용 가능하며 ID 통합 이전에 사용하던 ID는 사용할 수 없습니다. 기한 내에 ID 통합 절차를 거치지 않은 회원이 사용하던 ID는 2012년 11월 1일 자동 폐기되며, 계속해서 회원 서비스를 이용하고자 할 경우 새로운 ID를 생성해야 합니다.
...
"휴면회원" : [무비] 홈페이지에 최종 로그인 한 후 연속하여 36개월 간 로그인 기록이 없는 회원을 말합니다.
<br /><br />
제 5조. 이용 계약의 성립
...
1) 무비 사이트 유료 컨텐트 이용, 쇼핑몰에서의 물품구매 등 서비스 제공을 위해 이용자의 이름, 주소, 전화번호 등이 해당 유료 컨텐트 제공자, 쇼핑몰 업체, 배송업자에게 제공될 수 있습니다.
2) 무비 사이트 내에서 벌어지는 각종 이벤트 행사에 참여한 회원의 개인정보가 해당 이벤트의 주최자에게 제공될 수 있습니다.
...
온라인 예매는 1회 8매까지로 한정하며, 구매 횟수는 1일 10회로 제한됩니다. 단, 현장 구매 시에는 매수를 제한하지 않습니다.
...
멤버십 포인트는 최초 적립된 시점에서 24개월이 지난 시점의 해당 월 말일에 먼저 적립된 포인트부터 월 단위로 자동 소멸됩니다.
당사는 회원에게 제공되는 포인트의 소멸시효가 도래하여 포인트를 소멸시키는 경우, 포인트 소멸과 관련된 내용을 최소 15일 전 e-mail 또는 SMS를 발송하여 홈페이지에서 해당월 소멸 예정 포인트 확인이 가능하도록 조치 합니다.
매월 1일 오전 4시부터 7시까지는 전산 작업으로 인해 포인트 사용 및 취소가 불가능 할 수 있습니다.
...
단체관람을 통해 할인을 받는 경우 포인트는 적립되지 않습니다.
현장에서 티켓 소지자가 포인트 적립자 변경 요청 시, 해당 티켓을 예매한 계정에서 실 관람자가 요청한 계정으로 포인트 적립이 변경됩니다. (영화 상영시작 전 요청한 건에 한함)
제 14조. 정보의 제공
...
제 15조. 자격 상실 및 회원 서비스 혜택 제한
회원이 다음 각 호에 해당하는 경우 회원 자격이 상실되거나 서비스 혜택이 제한될 수 있다.
이 경우 회사는 그 사유를 회원에게 등록된 이메일 또는 모바일 메시지를 통해 통보합니다.
입회신청서의 기재사항을 허위로 작성한 경우
포인트 적립이나 영리의 목적으로 티켓을 발권한 다음 다른 사람에게 대가를 받고 양도하는 경우
다른 사람이 영화 관람 후 버린 티켓을 습득하여 포인트를 적립하는 경우
홈페이지나 모바일 화면에서 타인의 명예를 손상시키거나 불이익을 주는 행위를 한 경우
서비스 운영을 고의로 방해하는 행위를 하는 경우 (통보 후 3일의 사전 유예기간 부여)
[휴면회원]에 대해 회원자격 상실에 대한 안내문(전자우편 또는 서면)을 통지하고, 안내문에서 정한 기한 내에 답변이 없을 경우 회원자격을 상실시킬 수 있습니다.
...
부칙
(시행일) 이 약관은 2015년 5월 26일부터 시행합니다.
(변경) 종전의 약관은 본 약관으로 대체합니다.
	...
	<!-- 약관2 -->
		...
		<dl>
				...
				&lt;필수입력사항&gt;<br />
				성명, 아이디, 비밀번호, 휴대폰번호, 이메일 생년월일, SMS/이메일 수신여부, 본인확인기관을 통해 받는 결과값(CI) 등<br /> 
				<br>
				&lt;선택입력항목&gt;<br />
				거주지 주소, 유선 전화번호, 선호극장, 페이스북 계정<br /> 
				<br>
				&lt;서비스 이용 또는 사업처리 과정에서 생성 수집되는 각종 거래 및 개인 성향 정보&gt;<br />
				서비스 이용기록, 접속로그, 쿠키, 접속IP정보, 결제기록 등<br />
				<br />
				&lt;수집ᆞ이용목적&gt;<br />
		...
		<dl>
				...
				<dd> 2. 회원은 위 표와 같은 목적으로 제3자에게 본인의 개인정보를 제공하는 것에 동의하지 않을 수 있으며, 제3자 정보제공 및 활용 동의 여부와 관계없이 메가박스에 가입할 수 있습니다.</dd>
				<dd> 3. 메가박스와 관련한 제휴 계약 등을 신규 체결 또는 상기 기재된 개인정보 제공업체와의 기존 계약을 해지할 수 있으며, 그 경우 개인정보를 제공받는 업체에 변경이 생길 수 있습니다. 변경사항은 발생 시 홈페이지를 통해 고지하거나 동의를 구한 후 업체에게 제공됩니다.</dd>
				<dd> 4. 회원은 제휴업체에 대한 정보 제공 철회(제휴업체 이용약관 동의 철회)를 원하시는 경우 각 제휴업체에 방문하여 이용약관 철회 및 정보 제공 철회를 직접 진행해주셔야 합니다.</dd>
				<dd> 5. 제공받는 업체 모두에게 개인정보가 제공되지 않으며 회원님의 추가 동의 및 요청에 따라 정보가 해당 업체에 제공됩니다.</dd>
				<dd> 6. 개인정보 제공에 대한 동의를 거부할 권리가 있습니다. 단, 이를 거부하실 경우 제공받는 업체(모바일앱)를 통한 멤버십 서비스는 이용이 불가합니다.</dd>
			</dl>
		</div>
